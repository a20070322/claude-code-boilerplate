...
- ❌ 不要在 Store 中直接调用 `uni.xxx` API（应该在 composable 中）
- ❌ 不要在 Store 中进行复杂的业务逻辑（应该在 composable 中）
- ❌ 不要在 getters 中进行异步操作
- ❌ 不要在 Store 中直接修改其他 Store 的状态（使用 action）
- ❌ 不要持久化敏感数据（如密码、token）
- ❌ 不要在 state 中存储大对象（影响性能）
...
- [ ] 敏感数据不持久化
- [ ] 使用 `storeToRefs` 保持响应性（在 composable 中）
- [ ] 有清晰的注释说明用途
